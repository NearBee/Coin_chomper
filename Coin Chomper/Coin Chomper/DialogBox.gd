...
func animate_in():
	...
	tween.interpolate_property_over_time(self, 'modulate:a', 0.0, 1.0, 0.5, Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)
	tween.start()
...
func animate_out():
	var tween = get_tree().create_tween()
	tween.interpolate_property_over_time(self, 'modulate:a', 1.0, 0.0, 0.5, Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)
	tween.start()
